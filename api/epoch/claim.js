...
export default async function handler(req, res) {
  try {
    ...
    let ymd = String(body.ymd || '').trim();
    if (!ymd) ymd = await getLastSettledYmd();
    if (!ymd) return res.status(404).json({ error: 'no_settled_day' });
    ...
    if (!raw) {
      ...
      await bump('epoch_claim', 200);
      return res.status(200).json({ ok: true, ymd, address, amountUsd: 0, record: null, claimed: true, note: 'no_rewards' });
    }
