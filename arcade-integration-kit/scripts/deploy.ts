...
// Hardhat 3 compatibility helper - create ethers wrapper from connection config
async function getEthersFromConnection(connection: any) {
  ...
  if (!rpcUrl) throw new Error("No RPC URL found in network config");
  if (accounts.length === 0) throw new Error("No accounts found in network config");
  ...
  return {
    ...
    getContractFactory: async (name: string) => {
      // Load artifact from hardhat artifacts folder
    ...
    ZeroAddress: ethersLib.ZeroAddress,
  };
}
...
async function main() {
  ...
  // Use Hardhat 3 compatible ethers helper
