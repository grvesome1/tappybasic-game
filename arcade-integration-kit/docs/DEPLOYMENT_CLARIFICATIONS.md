...
Scope note:
- This document describes deploying the *full gruesome.arcade web stack* (Vercel static + serverless + Redis/KV + WalletConnect + PoH) alongside contract deploy/verify.
- This repo is the **contracts / integration kit** (Hardhat + manifests + SDK/backend-kit). If you are not working in the main web repo, treat the Vercel-specific sections as informational only.
...
Clarification:
- `/api/session/establish` requires Redis configured.
- The access gate calls `/api/session/establish` after PoH verify (best-effort).

### 5.4 WalletConnect
Current behavior:
- WalletConnect project id is currently embedded in the access gate HTML.

Recommendation:
- For clean staging/prod separation, implement an env-driven config endpoint and have the gate load it.

Minimum requirement:
- WalletConnect Cloud project must allow the domain(s):
